...
const FIELDS_FOR_CREDENTIALS = [ 'accessKeyId', 'secretAccessKey', 'bucketName', 'host'];
...
const init = (credentials) => {
  ...
  show.info('[config] Read credentials');
  ...
  if (credentials.host) {
    const ep = new AWS.Endpoint(credentials.host);
    s3 = new AWS.S3({endpoint: ep, signatureVersion: 'v2'});
    show.info('[s3] S3 initialized');
  } else {
    s3 = new AWS.S3();
